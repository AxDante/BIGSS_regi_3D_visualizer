...
class ControlPanel:
    ...
    def on_generate_drr(self):
        ...
        # Get Global Transform (LPS)
        # The visualizer is in LPS, so we can use the matrix directly.
        matrix_lps = np.array(plane_obj.actor.user_matrix)
    ...
    def on_refresh_drr(self):
        ...
        try:
            ...
            # Priority 1: Side-car file (if generated by service in output dir)
            # Priority 2: /tmp/drr_metadata.txt (default location)
            txt_file = latest_file.replace(".png", ".txt")
            if not os.path.exists(txt_file):
                txt_file = "/tmp/drr_metadata.txt"
            
            if os.path.exists(txt_file):
                logging.info(f"Found metadata file at: {txt_file}")
                self.update_c_arm_from_metadata(txt_file)
            else:
                logging.warning(f"No metadata file found. Checked {latest_file.replace('.png', '.txt')} and /tmp/drr_metadata.txt")
                
        except Exception as e:
            logging.error(f"Error loading DRR image: {e}")

    def update_c_arm_from_metadata(self, metadata_path):
        try:
            logging.info(f"Reading metadata from {metadata_path}...")
            with open(metadata_path, 'r') as f:
                data = json.load(f)
            
            logging.info(f"Metadata content: {json.dumps(data, indent=2)}")
                
            camera_pose = np.array(data.get('camera_pose'))
            intrinsics = data.get('intrinsic_params', {})
            ...
            if current_plane_sel:
                ...
                for p_conf in self.viz.config.get('reference_planes', []):
                    if p_conf['name'] == current_plane_sel:
                        parent_frame_name = p_conf['parent']
                        ref_plane_obj = self.viz.object_map.get(parent_frame_name)
                        break
            
            if ref_plane_obj:
                logging.info(f"Reparenting C-arm objects to {ref_plane_obj.name}")
            else:
                logging.warning(f"Could not find parent frame for reference plane {current_plane_sel}. C-arm will remain in World frame.")

            # Update CArmSource
            c_arm_obj = self.viz.object_map.get('CArmSource')
            if c_arm_obj:
                # Reparent if possible
                if ref_plane_obj:
                    c_arm_obj.set_parent(ref_plane_obj)
                    
                    # Calculate Local Transform (Plane_from_Camera)
                    # T_local = inv(T_plane_global) @ T_camera_global
                    # Note: camera_pose from metadata IS T_camera_global (World_from_Camera)
                    
                    plane_global = ref_plane_obj.global_transform.data
                    plane_inv = np.linalg.inv(plane_global)
                    
                    local_pose = plane_inv @ camera_pose
                    c_arm_obj.local_transform.data = local_pose
                    
                    logging.info(f"Updated CArmSource pose (Relative to {ref_plane_obj.name}).")
                else:
                    # Fallback to World Frame
                    c_arm_obj.local_transform.data = camera_pose
                    logging.info("Updated CArmSource pose (World Frame).")
                    
                c_arm_obj.update_transform(self.viz.transform_map)
            else:
                logging.warning("CArmSource object not found in scene.")

            # Update DRRDetector
            detector_obj = self.viz.object_map.get('DRRDetector')
            if detector_obj:
                # Calculate Detector Pose (World Frame first)
                trans_mat = np.eye(4)
                trans_mat[2, 3] = sdd 
                detector_pose_world = camera_pose @ trans_mat
                
                # Reparent if possible
                if ref_plane_obj:
                    detector_obj.set_parent(ref_plane_obj)
                    
                    # Calculate Local Transform
                    plane_global = ref_plane_obj.global_transform.data
                    plane_inv = np.linalg.inv(plane_global)
                    
                    local_pose = plane_inv @ detector_pose_world
                    detector_obj.local_transform.data = local_pose
                    
                    logging.info(f"Updated DRRDetector pose (Relative to {ref_plane_obj.name}).")
                else:
                    detector_obj.local_transform.data = detector_pose_world
                    logging.info("Updated DRRDetector pose (World Frame).")
                    
                detector_obj.update_transform(self.viz.transform_map)
            else:
                logging.warning("DRRDetector object not found in scene.")
                
            # Trigger a scene update to ensure all dependencies are updated
            # self.viz.update_scene() # This might be too heavy, but ensures everything is consistent
            
        except Exception as e:
            logging.error(f"Error updating C-arm pose from metadata: {e}")
            import traceback
            traceback.print_exc()
